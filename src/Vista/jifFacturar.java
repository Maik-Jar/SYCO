...
public final class jifFacturar extends javax.swing.JInternalFrame {
            ...
            if (Validacionduplicidad(dtmtbProductos.getValueAt(fsel, 0).toString()) == true) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        registro[4] = rs.getDouble(3); // Precio
                    ...
                    registro[3] = Integer.parseInt("1"); // cantidad
                    registro[5] = Double.parseDouble("0.00"); // Descuento

                    if ("Si".equals(itbis)) {
                        registro[6] = Double.parseDouble(registro[4].toString()) * ITBIS; // ITBIS
                    } else {
                        registro[6] = "0.00"; // ITBIS
                    }
                    registro[7] = Double.parseDouble(registro[4].toString()) + Double.parseDouble(registro[6].toString()); // Monto
                ...
                if (tfPagado.getText() == null ? tfTotal.getText() != null : !tfPagado.getText().equals(tfTotal.getText())) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < dtmtbFacturacion.getRowCount(); i++) {
                            ...
                            ps.setInt(4, Integer.parseInt(dtmtbFacturacion.getValueAt(i, 3).toString())); // Envia la cantidad.
                            ps.setDouble(5, Double.parseDouble(dtmtbFacturacion.getValueAt(i, 4).toString())); // Envia el precio.
                            ps.setDouble(6, Double.parseDouble(dtmtbFacturacion.getValueAt(i, 5).toString())); // Envia el descuento.
                            ps.setDouble(7, Double.parseDouble(dtmtbFacturacion.getValueAt(i, 6).toString())); // Envia el ITBIS.
                            ps.setDouble(8, Double.parseDouble(dtmtbFacturacion.getValueAt(i, 7).toString())); // Envia el monto.
    ...
    private void tbFacturacionKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbFacturacionKeyPressed
        ...
        if (columna == 3) { // Columna Cantidad
            if (dtmtbFacturacion.getValueAt(fila, 3) == null || Integer.parseInt(dtmtbFacturacion.getValueAt(fila, 3).toString()) == 0) { // Evalua si la columna "Cantidad" esta vacia o tiene Cero.
        ...
        if (columna == 5) { // Columna Descuento
            if (dtmtbFacturacion.getValueAt(fila, 5) == null) { // Evalua si la columna "Descuento" esta vacia.
                dtmtbFacturacion.setValueAt(0.00, fila, 5); // Si la columna "Descuento" esta vacia, asigna un valor de 0.0.
                dtmtbFacturacion.fireTableDataChanged(); // Ejecuta el cambio.
            }
            ...
            if (Double.valueOf(dtmtbFacturacion.getValueAt(fila, 6).toString()) != 0.0) {
    ...
    private void btGenerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btGenerarActionPerformed
        ...
        if (dtmtbFacturacion.getRowCount() == 0) {
            ...
            for (int i = 0; i < dtmtbFacturacion.getRowCount(); i++) {
                cotizacion = new Cotizacion(lbTipodefactura.getText(), icono.getImage(), cbDocumento.getSelectedItem().toString() + ":", tfBusqueda.getText(), tfCliente.getText(), tfTelefono.getText(), tfCodcliente.getText(), currentTime, dtmtbFacturacion.getValueAt(i, 1).toString(), dtmtbFacturacion.getValueAt(i, 2).toString(), dtmtbFacturacion.getValueAt(i, 3).toString(), dtmtbFacturacion.getValueAt(i, 6).toString(), dtmtbFacturacion.getValueAt(i, 4).toString(), tfSubtotal.getText(), tfDescto.getText(), tfITBIS.getText(), tfTotal.getText(), usr.getNombre(), taComentario.getText());
    ...
    private void CalcularTotal() {
        ...
        if (dtmtbFacturacion.getRowCount() > 0) {
            for (int i = 0; i < tbFacturacion.getRowCount(); i++) {
                precio[0] = Double.parseDouble(dtmtbFacturacion.getValueAt(i, 4).toString()); // Precio
                total[0] += precio[0];
            }
            tfSubtotal.setText(String.valueOf(formatodecimal.format(total[0])));
            
            for (int i = 0; i < tbFacturacion.getRowCount(); i++) {
                precio[1] = Double.parseDouble(dtmtbFacturacion.getValueAt(i, 6).toString()); // ITBIS
                total[1] += precio[1];
            }
            tfITBIS.setText(String.valueOf(formatodecimal.format(total[1])));
            
            for (int i = 0; i < tbFacturacion.getRowCount(); i++) {
                precio[2] = Double.parseDouble(dtmtbFacturacion.getValueAt(i, 5).toString()); // Descuento
                total[2] += precio[2];
            }
            tfDescto.setText(String.valueOf(formatodecimal.format(total[2])));
            
            for (int i = 0; i < tbFacturacion.getRowCount(); i++) {
                precio[3] = Double.parseDouble(dtmtbFacturacion.getValueAt(i, 7).toString()); // Monto
                total[3] += precio[3];
            }
        ...
        for (int i = 0; i < dtmtbFacturacion.getRowCount(); i++) {
            try {
                ...
                if (!(chbPreventa.isSelected())) {
                    ...
                    ps.setInt(2, cantidad[i] - Integer.parseInt(dtmtbFacturacion.getValueAt(i, 3).toString())); // Resta la cantidad en base de datos - cantidad facturada.
    ...
    private void Habilitar_campos_inf_cliente () {
